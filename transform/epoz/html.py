...
# function to add p elements around the contents of a tag if there
# isn't one yet, required because Silva expects 'p' elements around
# the contents of a number of tags
def add_ps_if_required(el, context):
        converted = el.content.convert(context)
        if len(el.content) > 1 or (len(el.content) and el.content[0].name() != 'p'):
            # add p's around non-p content
            converted = [silva.p(el.content.convert(context))]
        return converted
...
class body(Element):
    ...
    def convert(self, context):
        ...
        # the add_ps_if_required function can not be used here because
        # some specific issues need to be taken care of we don't want 
        # to support in that helper
        fixedrest = []
        textbuf = []
        for el in rest:
            if el.name() not in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table', 'p', 'pre', 'img', 'ul', 'ol', 'dl']:
        ...
        return silva.silva_document(
                silva.title(title),
                silva.doc(
                    fixedrest # rest.convert(context)
                ),
                id = id
            )
...
class table(Element):
    def convert(self, context):
        ...
        return silva.table(
                self.content.convert(context),
                columns=self.attrs.get('highest', cols),
...
class td(Element):
    def convert(self, context):
        converted = add_ps_if_required(self, context)
        return silva.field(
                converted
        )
