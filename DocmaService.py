...
class DocmaService(SimpleItem):
    ...
    def silva2word(self, email, xml, fronttemplate, doc, username, description):
        """Silva to Word conversion"""
        server = xmlrpclib.Server("http://%s:%s" % (self._host, self._port))
        (ident, storageid) = server.silva2word(username, self._password, email, "sample", fronttemplate[:fronttemplate.rfind('.')], xmlrpclib.Binary(xml), description)
